...
<body class="backroundCol">
  ...
  <!-- table-bordered -->
  <table class="table table-responsive mt-5" style="text-align:center; vertical-align: middle; background-color: #4A2815; color: white; font-size: 26px; margin-bottom: 300px;">
    ...
    <tbody>
      <tr>
        ...
        $query = "SELECT TotalCost FROM CART WHERE CartID=$Cid";
        $results = sqlsrv_query($conn, $query);
        ...
        echo '<td>';
        echo '<p class="p-price">'.PrintResultFloatNumber($results).'</p>';
        echo '</td>';
        echo '<td>';
        echo '<button class="btn btn-dark" type="button">Proceed to Check Out</button>';
        echo '</td>';
        echo ("</tr>"); 
...
function PrintResultSet($resultSet){
  ...
  while ($row = sqlsrv_fetch_array($resultSet, SQLSRV_FETCH_ASSOC)) {
    ...
    foreach ($row as $col) {
      ...
      echo  '<img class="img-fluid mb-1" style="height:90px; width:90px;" src="' . $col . '" alt="Image"/>
          </a>';
      else if ($cnt == 3){
        echo '<a href="#" class="btn btn-sm btn-dark me-2">+</a>';
        echo $col;
        echo '<a href="#" class="btn btn-sm btn-dark ms-2">-</a>';
      }
